/**
 * The branded shell for the emails Medusa sends — the activity and workshop
 * booking confirmations and the farm's booking notices (src/lib/booking-payment.ts):
 * the logo on top, a honey-gold header bar with the heading, a white card for
 * the body, and the shop's contact line underneath.
 *
 * A copy of the storefront's src/lib/email-layout.ts, which wraps the order and
 * enquiry emails — the two apps share no code, so keep them looking the same.
 *
 * Email clients are not browsers: layout is nested tables, every style is
 * inline, fonts are the system stack, and the logo is a transparent PNG served
 * by the storefront (Gmail and Outlook block SVG).
 */
...
/** The public storefront origin email clients fetch the logo from — never localhost. */
export function assetOrigin(): string {
  const site = process.env.STOREFRONT_URL?.replace(/\/$/, '')
